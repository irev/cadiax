...
def test_cli_service_show_renders_posix_worker_artifacts(tmp_path, monkeypatch):
    ...
    assert "[autonomiq-worker.service]" in result.output
...
def test_cli_service_show_renders_dashboard_artifacts(tmp_path, monkeypatch):
    ...
    assert "[autonomiq-dashboard.service]" in result.output
...
def test_cli_service_write_generates_wrapper_files(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0
    assert (output_dir / "autonomiq-worker.service").exists()
    assert (output_dir / "autonomiq-worker.sh").exists()
...
def test_cli_doctor_reports_missing_remote_provider_credential(tmp_path, monkeypatch):
    ...
    assert result.exit_code == 0
    assert "Autonomiq Config Status" in result.output
    ...
    assert "Jalankan `autonomiq setup`" in result.output
...
def test_assistant_supports_doctor_and_config_status_commands(tmp_path, monkeypatch):
    ...
    assert "Autonomiq Config Status" in doctor_result
    ...
    assert "Autonomiq Config Status" in alias_result
