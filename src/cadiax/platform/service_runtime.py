...
def run_named_service_target(
    ...
    *,
    ...
    from cadiax.core.assistant import Assistant
    from cadiax.services.interactions import ConversationService
    from cadiax.services.interactions import run_conversation_api
...
def _build_posix_artifacts(spec: ServiceTargetSpec, args: list[str]) -> list[ServiceArtifact]:
    ...
    unit = "\n".join(
        [
            "[Unit]",
            f"Description=Cadiax {spec.name} service",
...
def _build_windows_artifacts(spec: ServiceTargetSpec, args: list[str]) -> list[ServiceArtifact]:
    ...
    install = "\n".join(
        [
            ...
            "-Settings $Settings -Description 'Cadiax generated service wrapper' -Force",
            "",
        ]
    )
    prefix = f"cadiax-{spec.name}"
