...
def _run_interactive(service: ConversationService) -> None:
    click.echo(f"Autonomiq v{__version__}")
    click.echo("Type 'help' for available commands, 'exit' to quit.")
    if should_recommend_setup():
        click.echo("Konfigurasi awal belum lengkap. Jalankan `autonomiq setup` untuk wizard interaktif.")
    click.echo("")
...
@click.option(
    "--setup",
    is_flag=True,
    help="Compatibility alias for `autonomiq setup`",
)
@click.option(
    "--doctor",
    is_flag=True,
    help="Compatibility alias for `autonomiq doctor`",
)
@click.pass_context
def main(
    ctx: click.Context,
